...
function cleanPitchName(name) {
  ...
  const pitch = name.toLowerCase();

  if (pitch.includes("fastball") || pitch.includes("four-seam") || pitch.includes("4-seam") || pitch.includes("sinker")) {
    return "Fastball";
  }

  if (pitch.includes("change")) return "Changeup";
  if (pitch.includes("slider")) return "Slider";
  if (pitch.includes("curve")) return "Curveball";
  if (pitch.includes("split")) return "Splitter";

  return null;
}

function calculateTrackManMetrics(rows) {
  const player = ensurePlayer(selectedPitcher);
  ...
  rows.forEach(row => {
    const pitchName = cleanPitchName(row.TaggedPitchType || row.PitchType || row.AutoPitchType);

    if (pitchName) {
      pitchGroups[pitchName].push(row);
    }
  });

  player.pitchMetrics = {};

  Object.keys(pitchGroups).forEach(pitch => {
    const pitches = pitchGroups[pitch];
    ...
    player.pitchMetrics[pitch] = {
      ...
      releaseHeight: avg(pitches.map(p => p.RelHeight)),
      spinRate: avg(pitches.map(p => p.SpinRate)),
      extension: avg(pitches.map(p => p.Extension)),
      releaseTilt: avg(pitches.map(p => p.SpinAxis))
    };
  });
...
document.querySelector('input[type="file"]').addEventListener("change", function(event) {
  ...
  reader.onload = function(e) {
    ...
    calculateTrackManMetrics(rows);

    alert("TrackMan data uploaded and sorted.");
  };
